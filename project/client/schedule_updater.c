...
void schedule_updater_pkt_decode(struct schedule_updater_pkt *pkt, const uint8_t *pkt_raw) {
    ...
    for (uint8_t i = 0; i < pkt->cell_ids_len; i++) {
        ...
        memcpy(&pkt->cell_ids[i].timeslot, pkt_raw+index, sizeof(pkt->cell_ids[i].timeslot));
        index += sizeof(pkt->cell_ids[i].timeslot);
        memcpy(&pkt->cell_ids[i].channel, pkt_raw+index, sizeof(pkt->cell_ids[i].channel));
        index += sizeof(pkt->cell_ids[i].channel);
    }
}
...
void schedule_updater_pkt_log(struct schedule_updater_pkt *pkt) {
    ...
    LOG_INFO_LLADDR(&pkt->neighbor_addr);
    LOG_PRINT("\n");
