...
{ cell(N, M, S, C) } B :- totalBandwidth(B), node(N), node(M), N != M, ingraph(N, M), S = 1..nSlots, C = 1..nChannels.
...
% Remove models that doesn't satisfy the bandwidth requirements.
:- node(N), bandwidth(N, B), not B { cell(N, _, _, _) }.
